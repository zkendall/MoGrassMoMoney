...
- Verified by small-map asset generation, `test:all`, and a Playwright client screenshot pass.
...
- Verified by `test:all` and a Playwright client pass with no browser errors.
...
- Verified by all-map asset generation, `test:all`, and Playwright client screenshot passes for `medium` and `large`.
...
- Verified by verify-script syntax checks.
- Verification:
...
- Verification: Verified by updated-script `node --check`, `test:rng`, and `test:regression`.
...
- Verification: Verified by verify-runner syntax checks and a `verify:quick` pass.
- Verification: Verified by `verify:syntax`, `test:rng`, `test:regression`, and `verify:quick`.
- Verification: Measured runtimes for `verify:syntax`, `test:rng`, `test:regression`, and `verify:quick`.
...
- Verification: Verified by runner `node --check` and a timed `verify:quick` pass (`real 8.72s`).
...
- Verification: Verified by runner syntax checks, `test:regression`, and `verify:quick`.
...
- Verification: Verified by label/runner syntax checks, `test:regression`, and `verify:quick`.
...
- Verification: Verified by runner syntax checks, `verify:syntax`, `test:rng`, `test:regression`, and `verify:quick`.
...
- Verification: Verified by a section-heading presence check in `PLAYWRIGHT-TEST-MIGRATION-PLAN.md`.
...
- Verification: Verified by `test:regression`.
- Verification: Verified by `test:regression:update`.
- Verification: Verified by `test:rng`, `verify:syntax`, and `verify:quick`.
...
- Verification: Verified by `test:regression`.
- Verification: Verified by `verify:quick`.
- Verification: Verified by the full regression runner, `verify:syntax`, and `test:rng`.
...
- Verification: Verified by headed `verify:quick`.
...
- Verified by `verify:syntax`, `test:rng`, a direct Playwright regression run, a quick-suite compatibility run, and `verify:quick`.
...
- Verified by `verify:syntax`, `test:rng`, a direct Playwright regression run, and a direct Playwright quick-verify run.
...
- Verified by checking docs and CI for removed redundant `TYCOON_BASE_URL` overrides.
...
- Verified by Playwright spec/helper `node --check` passes and `test:rng`.
...
- Verified by Playwright spec `node --check` passes.
...
- Verified by Playwright spec/state `node --check` passes, `test:regression`, and `verify:quick`.
...
- Verified by a README and `GAME-DESIGN.md` section-heading presence check.
...
- Verified by an upgrade-section presence check in `GAME-DESIGN.md`.
...
- Verified by a `Summary.md` section-heading presence check.
...
- Verified by `test:quick`, `test:visual`, and `test:all`.
...
- Verified by a neighborhood-navigation doc reference check.
...
- Verified by a mowing docs keyword-presence check.
...
- Verified by long-playthrough syntax and fixture checks, `verify:long`, `test:regression`, and `verify:quick`.
...
- Verified by shared harness and spec `node --check` passes, `test:regression`, `verify:quick`, and `verify:long`.
...
- Verified by mowing runtime/spec `node --check` passes and `test:all`.
...
- Verified by mowing runtime `node --check` and `test:all`.
...
- Verified by `test:all` and Playwright client browser passes for `small`, `medium`, and `large`.
...
- Verified by grass-asset generation, Python compile check, `test:visual`, and `test:quick`.
...
- Verified by `test:visual` and `test:quick`.
...
- Verified by `test:all` and a Playwright browser snapshot pass.
...
- Verified by `test:all` and a Playwright browser pass.
...
- Verified by `test:visual` and a Playwright client grass-autotile pass.
...
- Verified by `test:visual` and a Playwright client grass-autotile cleanup pass.
...
- Narrowed the small gas mower cut width in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/constants.js` by reducing `deckRadius` from `26` to `20` so its mow strip fits the visible sprite better, and updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/menu-setup.spec.js` with a direct gas-mower regression plus a corrected manual-mower setup assertion.
- Verified by `npm --prefix mowing-poc-game run test:all -- menu-setup.spec.js`.
- Added HTML entrypoint cache busting in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html` so normal refreshes request fresh `styles.css`, `maps.js`, and `game.js` builds; synced the stale small-gas width expectation in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/menu-setup.spec.js` back to the current `deckRadius` of `16`.
- Verified by `npm --prefix mowing-poc-game run test:all -- menu-setup.spec.js`.
- Reverted the HTML entrypoint cache-busting changes in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html` after confirming the browser's DevTools cache toggle, not query-string injection, was the reliable fix for local reload behavior.
- Replaced the mowing POC dev server with `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/scripts/dev_server.py`, updated Playwright and `npm run dev` to use it, and documented the no-cache local workflow in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md`.
- Verified by `python3 -m py_compile mowing-poc-game/scripts/dev_server.py`, `npm --prefix mowing-poc-game run test:all -- menu-setup.spec.js`, and `curl -I http://127.0.0.1:4173/`.
- Swapped `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game` grass rendering to split `16x16` strip assets: renderer now loads `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-unmowed.png` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-mowed.png`, removed the old combined-sheet row selection and skirt/overlap assumptions, and synced debug/test/README metadata. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js` and the Playwright web-game client against `http://127.0.0.1:4173`.
- Varied grass tiling in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/render/scene.js` by rotating only autotile column `0` in deterministic `90` degree steps per grid cell, reducing visible repetition without per-frame flicker. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js`.
- Replaced the simple grass coordinate mix in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/render/scene.js` with a stronger integer hash of tile row/col before picking the quarter-turn, giving column `0` placements a less regular spatial pattern while staying deterministic. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js`.
- Added a grass sprite debug overlay mode in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game`: `G` and `window.setGrassSpriteDebug(true/false)` now toggle centered frame-index labels on each grass tile, and the debug payload reports whether the overlay is enabled. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js`.
- Replaced the per-tile grass debug labels in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game` with a cursor-driven debug panel overlay: when `G` or `window.setGrassSpriteDebug()` is enabled, the UI now shows the hovered tile's row/col, state, frame index, and rotation instead of stamping numbers onto every grass cell. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js`.
- Fixed grass debug panel visibility in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/render/ui.js` so toggling `G` also shows the panel while still on the setup menu instead of only after leaving menu mode; updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-stripes.spec.js` to cover the menu-state toggle. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js`.
- Changed `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/render/scene.js` grass frame ownership to directional state differences instead of lower-height checks, so the existing south/east/southeast transitions stay intact while mowed tiles can also pick non-zero frames when unmowed grass is below/right; exposed `window.getGrassDebugInfoAt()` for debugging and added a targeted bottom-boundary regression in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-stripes.spec.js`. Verified by `npm --prefix mowing-poc-game run test:all -- visual-stripes.spec.js`.
- Documentation note: added [Piskel](https://www.piskelapp.com/) to `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` as a good browser-based editor for pixel art grass assets.
- Verified by `test:visual` and a Playwright client push-width pass.
- Note: the menu-setup Playwright check was blocked by sandboxed Chromium launch permissions.
- Cleaned up overly detailed verification notes in `/Users/zkendall/projects/MowGrassMoMoney/progress.md` to keep entries short and outcome-focused.
