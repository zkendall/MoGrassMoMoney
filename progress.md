Original prompt: Build a simple game based on the POC-Mowing.md directions. Keep the broader context in mind such as Summary.md. Place the game in a sub-folder.

- Parsed requirements from POC-Mowing.md and Summary.md.
- Decided scope: single-scene top-down mowing POC with mouse-hold movement, steering by cursor offset, obstacle collisions, mow coverage % and 95% win condition.
- Chosen implementation path: plain HTML/CSS/JS in `mowing-poc-game` for quick iteration.

- Created `mowing-poc-game` with `index.html`, `styles.css`, and `game.js`.
- Implemented single-lawn scene, mower movement, mouse steering, reverse, obstacle collisions, mow-state grid, coverage UI, and 95% completion condition.
- Exposed deterministic hooks: `window.render_game_to_text` and `window.advanceTime(ms)`.
- Next: run Playwright client loop, inspect screenshot/state, and fix any issues.

- Installed Playwright dependencies needed by `develop-web-game` skill client and downloaded Chromium runtime.
- Ran Playwright loop against `http://127.0.0.1:4173` with multi-step mouse input bursts.
- Reviewed generated screenshots in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/output/web-game`:
  - Mowed vs unmowed grass readability is clear and immediate.
  - Mower movement/steering/reverse interactions function and update coverage in real time.
  - Obstacles and lawn bounds block traversal as expected.
- Verified `render_game_to_text` outputs deterministic state snapshots (`state-0.json`..`state-2.json`) including coordinates, mower state, map obstacles, objective, and coverage.
- No console/page errors were generated by the Playwright client (no `errors-*.json` files emitted).

TODO / Next suggestions:
- Tune steering/speed values after hands-on play if you want a tighter or more arcade-like feel.
- Optional: add a simple time limit mode and medal score (coverage + time) while keeping this POC scope intact.
- Updated steering model in `mowing-poc-game/game.js` to use mouse X delta (left/right motion) instead of turning toward cursor position.
- Steering now works while stationary in `playing` mode; if mouse is not moved, mower maintains heading and drives straight.
- Added image-based mower rendering by integrating `mowing-poc-game/assets/mower-sheet.png`.
- Updated draw path in `mowing-poc-game/game.js` to render a selected sprite frame with rotation, with previous vector mower retained as fallback if image loading fails.
- Ran Playwright sprite sanity check and verified sprite appears in gameplay screenshot (`output/web-game-sprite-check/shot-0.png`).
- Increased mower steering sensitivity so each mouse move causes a higher turn rate (changed steer delta scaling from `/20` to `/10`).
- Added steering HUD slider (left-center-right) on canvas UI to visualize live steering input.
- Slider knob now reflects steering axis and recenters when mouse steering input stops.
- Exposed `input.steering_axis` in `render_game_to_text` for debugging parity with HUD.
- Changed steering to a persistent axis model: mouse movement now shifts steering left/right and steering holds when mouse stops moving.
- Removed auto-centering behavior; user must steer back toward center to neutralize turning.
- Switched steering back to mouse-follow mode (cursor-relative turning) instead of persistent steering axis.
- HUD steering slider now reflects live cursor-follow steering value again.
- Reworked collision response to be more forgiving with slide behavior.
- Added movement solver that advances to contact point, computes collision normal, and projects remaining movement along the surface.
- Reduced hard-stop feel by damping speed less aggressively on blocked movement.
- Increased mower turning sharpness by raising base `turnRate` from `2.3` to `3.4`.
- Increased grass mow chunk size by changing mow-grid cell size from `10` to `18` pixels (larger mowable chunks, easier coverage).
- Fixed mower sprite facing direction by flipping sprite heading offset from `+PI/2` to `-PI/2` so the mower points the correct way while moving.
- Added procedural background music using Web Audio API (no external audio files).
- Music starts on first user interaction and loops with a simple bass/pad pattern.
- Added mute toggle with `M` and HUD status text (`Music: On/Off`).
- Updated steering pivot model so heading rotates around a point near the mower front instead of center.
- Implemented front-pivot turn helper with collision-safe fallback to partial center rotation if blocked.
- Set music default to off by initializing `state.musicMuted` and `music.muted` to `true`.
- Added real grass sprite assets: `mowing-poc-game/assets/grass-unmowed.png` and `mowing-poc-game/assets/grass-mowed.png`.
- Updated `drawMowGrid()` to render per-cell grass via image sprites (with existing color fallback if images fail to load).
- Validated via Playwright screenshot: `mowing-poc-game/output/web-game-grass-sprites/shot-0.png`.
- Regenerated grass sprites per updated art direction:
  - `grass-mowed.png` is now a single flat tone (no stripe pattern).
  - `grass-unmowed.png` includes visible strand/blade texture.
- Verified in screenshot: `mowing-poc-game/output/web-game-grass-fix/shot-0.png`.

- Built text-first tycoon meta POC in new folder `tycoon-poc-text` with:
  - `index.html`, `styles.css`, `game.js`, and `README.md`.
  - Multi-day loop states: `planning` -> `performance` -> `report`.
  - Customer/job model including `pattern_preference` (`circle`/`stripe`/`none`).
  - Resolution math with quality multiplier, pattern modifier (+5% match / -5% mismatch except `none`), fuel + maintenance costs, churn + conversion handling, and upgrade purchasing (3 mower tiers).
  - Deterministic hooks: `window.render_game_to_text()` and `window.advanceTime(ms)`.

- Adjusted controls to be compatible with the Playwright skill client key map:
  - Planning: Up/Down cursor, Space toggle, Enter confirm.
  - Performance: Up/Down score, Left/Right cycle pattern, Enter resolve.
  - Report: `A` (or `U`) buy upgrade, Enter next day.

- Ran automated Playwright loop against `http://127.0.0.1:4174` and validated output artifacts in `tycoon-poc-text/output/web-game`:
  - Screenshots: `shot-0.png`..`shot-2.png`.
  - State snapshots: `state-0.json`..`state-2.json`.
  - State progression confirms day advancement and cash/upgrades changing across iterations (e.g., Day 2 -> Day 4, mower tier moved to `gas_push`).
  - No page error artifact files emitted.

TODO / Next suggestions:
- Add a dedicated seeded simulation mode endpoint (run N days without UI input) for faster economy balancing.
- Surface richer report details in `render_game_to_text` for direct assertion tests (per-job expected vs actual multipliers).
- Optionally port this text loop into the existing mowing POC as a pre/post-mow meta layer.
- Updated `tycoon-poc-text` layout orientation:
  - Console moved into left column.
  - Ongoing stats (canvas) moved into right panel with heading.
  - Added responsive breakpoint to stack panels on narrow screens.
- Verified app still renders and state output unchanged via Playwright artifacts in `tycoon-poc-text/output/web-game-layout`.

- Documentation sync for latest tycoon spec:
  - Updated `tycoon-poc-text/POC-Tycoon.md` to match current implementation.
  - Clarified current build target is browser-based text-first UI in `tycoon-poc-text`.
  - Updated interface spec to left-right workspace:
    - left console panel
    - right `Ongoing Stats` panel
    - right `Active Customers` panel below stats
  - Captured current controls:
    - Planning: Up/Down, Space, Enter
    - Performance: Up/Down, Left/Right, Enter
    - Report: A/U, Enter
    - Global: R, F
  - Captured implemented simulation features:
    - payout multipliers + pattern preference modifier
    - fuel/maintenance/net economy
    - retention/churn + new-customer conversion
    - 3-tier upgrades
    - deterministic hooks (`render_game_to_text`, `advanceTime`)

- UI updates reflected in spec:
  - Forced left-right panel orientation (no auto stack fallback).
  - Improved stats readability by resizing canvas rendering to panel size and increasing text scale.
  - Added `Active Customers` panel under stats with risk marker (`[risk]`) for delayed service.
- Updated tycoon customer acquisition flow:
  - Day 1 now starts with zero active repeat customers.
  - New customers are not auto-converted.
  - After resolving a day, passing new jobs (`final_score >= 70`) are shown as regular-customer offers in report phase.
  - Player can accept/decline offers (`Up/Down` + `Space`), and accepted offers are applied at next-day transition.
- Updated report controls and state payload:
  - Added offer cursor/selection handling in report mode.
  - Added `pending_regular_offers` in `render_game_to_text` payload.
- Updated docs/UI text for new controls and offer flow:
  - `tycoon-poc-text/index.html` hint text
  - `tycoon-poc-text/README.md`
  - `tycoon-poc-text/POC-Tycoon.md`
- Verification: Verified by Playwright run, deterministic probe.
- Implemented day-action decision system in `tycoon-poc-text/game.js`:
  - New start-of-day phase: `day_action` with options `Solicit`, `Follow Up Leads`, `Mow Lawns`.
  - Day 1 now starts with zero active repeat customers.
- Added lead pipeline model:
  - `state.leads[]` persists across days with `lead_status` (`raw`/`qualified`).
  - `Solicit` generates probabilistic raw leads and charges random materials cost.
  - `Follow Up Leads` attempts to convert raw leads to qualified leads.
  - `Mow Lawns` planning list now pulls from qualified leads + repeat customers only.
- Updated mowing conversion lifecycle:
  - Lead jobs with passing score (`final_score >= 70`) become regular-customer offers in report.
  - Player accepts/declines offers in report; accepted offers become repeat customers next day.
  - Serviced leads are removed from lead pool after mowing.
- Updated report accounting:
  - Non-mow actions now produce end-of-day reports with `activity` and `materials` cost fields.
  - `render_game_to_text` now includes `leads`, `day_action`, and updated controls text.
- Updated UI/doc text:
  - `tycoon-poc-text/index.html` hint text updated for day-action flow.
  - `tycoon-poc-text/README.md` updated with day flow + controls.
  - `tycoon-poc-text/POC-Tycoon.md` updated to match lead-based acquisition design.
- Verification: Verified by Playwright run, artifact output check, deterministic probe.
- Balancing tweak: reduced tycoon starting cash so day one begins below first mower upgrade cost.
  - Added `STARTING_CASH = 220` in `tycoon-poc-text/game.js`.
  - Applied value to initial state and reset path (`initialize()`).
- Note: attempted Playwright state verification was blocked by approval denial; code-level check confirms the configured value.
- Moved mower upgrade purchase from report phase into new day-action activity: `Shop for New Hardware`.
- `day_action` options now: `Solicit`, `Follow Up Leads`, `Mow Lawns`, `Shop for New Hardware`.
- Added `hardware_shop` mode:
  - `Up/Down` selects buy vs skip.
  - `Enter` confirms and ends the day.
  - Buying applies upgrade immediately and records `hardware_purchase`/`hardware_cost` in day report.
- Removed upgrade purchase keybinds from report mode.
- Updated docs/UI text:
  - `tycoon-poc-text/index.html`
  - `tycoon-poc-text/README.md`
  - `tycoon-poc-text/POC-Tycoon.md`
- Added timed processing transitions with spinner animation for time-consuming actions in `tycoon-poc-text/game.js`.
  - New transient mode: `processing` with animated spinner (`| / - \\`) and action label.
  - Added delays for:
    - `Solicit`
    - `Follow Up Leads`
    - transition to `Shop for New Hardware`
    - mowing result resolution (`performance` -> `report`)
    - hardware shop buy/skip confirmation
    - day advance (`report` -> next day)
- Input behavior updated:
  - While in `processing` mode, gameplay input is ignored (except reset/fullscreen handlers before mode checks).
- Verification: Verified by node --check, pass result check, verify-tycoon.sh.
- Verification workflow update:
  - `tycoon-poc-text/scripts/verify-tycoon.sh` now archives screenshots/state with global incremental numbering in creation order across runs.
  - Archive path: `tycoon-poc-text/output/verify-tycoon/snapshots`.
  - Added ordering guideline to `tycoon-poc-text/README.md`.
- Adjusted verification output indexing to match requested format.
  - `verify-tycoon.sh` now writes runs directly under `tycoon-poc-text/output` as:
    - `NN-web-game/`
    - `NN-probe.json`
  - Example: `01-web-game` + `01-probe.json`, then `02-*`, etc.
- Removed new nested numbering approach from the workflow; updated README guideline accordingly.
- Verification naming guideline update:
  - `verify-tycoon.sh` now names run folders/files as `NN-<change-label>-*`.
  - Label summarizes changes since the last successful verify baseline.
  - Falls back to `no-change` when no tracked changes are detected.
- Verification label tightening:
  - `verify-tycoon.sh` now generates shorter run labels meant to succinctly describe deltas since the previous verify run.
  - Preferred labels are compact categories (`gameplay`, `ui`, `docs`, `verify`) with file-based fallback and `no-change` when unchanged.
- Verification run:
  - Executed `./tycoon-poc-text/scripts/verify-tycoon.sh http://127.0.0.1:4174`.
  - Created indexed artifacts: `tycoon-poc-text/output/09-gameplay-ui-web-game` and `tycoon-poc-text/output/09-gameplay-ui-probe.json`.
  - Confirmed index-paired output structure remains intact.
- Day action UI readability update in `tycoon-poc-text/game.js`:
  - Added indented sub-description line under each day action row to summarize the current opportunity.
  - Sub-descriptions are dynamic (examples: raw leads available for follow-up, qualified+repeat jobs available for mowing, next hardware upgrade affordability).
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Added repo-local agent instruction file: `/Users/zkendall/projects/MowGrassMoMoney/AGENTS.md`.
  - Includes explicit requirement to read and append `/Users/zkendall/projects/MowGrassMoMoney/progress.md` after meaningful implementation, verification, or documentation changes.
- UI text spacing tweak in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/game.js`:
  - Increased indentation for day-action subheader lines (both description + opportunity) to improve visual hierarchy.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Balance adjustment in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/game.js`:
  - Reduced lead qualification chance during `Follow Up Leads` from `0.6` to `0.4`.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Refactor: split `tycoon-poc-text/game.js` into modular files under `tycoon-poc-text/src` with no intended behavior changes.
  - Added modules: `constants.js`, `state.js`, `stateMachine.js`, `jobs.js`, `dayActions.js`, `processing.js`, `keyboard.js`, `render/consoleView.js`, `render/statusPanel.js`, `render/activeCustomersView.js`, `index.js`.
  - Converted top-level `game.js` into a thin loader that imports `src/index.js`.
  - Centralized mode transitions via `stateMachine.js` (`transitionTo`/`forceMode`) to keep flow rules in one place.
- Docs update:
  - Added a `Code Structure` section to `tycoon-poc-text/README.md` describing the new module layout.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Balance adjustment in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js`:
  - Reduced `Solicit` lead generation chance from `0.68` to `0.45` to make lead acquisition harder.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Input compatibility fix in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/keyboard.js`:
  - Expanded selection key handling to accept `event.key === 'Space'` and `event.code === 'Space'` in addition to `' '` / `'Spacebar'`.
  - Applies to both planning job selection and report offer selection.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Clarified lead-state wording in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - `Follow Up Leads` opportunity now shows `raw leads`.
  - `Mow Lawns` opportunity now shows `qualified leads`.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Input flow update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/processing.js`:
  - Added optional `requireConfirm` parameter to `startProcessing` (default `true`).
- Added 16-direction manual push mower generation in `mowing-poc-game/scripts/generate_push_mower_sheet.py`, produced `mowing-poc-game/assets/push-mower-16dir.png`, and switched manual mower rendering in `mowing-poc-game/game.js` to use nearest-frame directional sprites with matched draw scale and text-state frame reporting.
- Verified by `python3 mowing-poc-game/scripts/generate_push_mower_sheet.py`, `python3 -m py_compile mowing-poc-game/scripts/generate_push_mower_sheet.py`, `node --check mowing-poc-game/game.js`, Playwright client run against `http://127.0.0.1:4173`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`.
- Added art-backed `small` map pilot in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game`: new map art contract, runtime base/mask loading, mask-driven mow grid, art diagnostics in `render_game_to_text`, deterministic guide/mask/base generator, and docs/tests for the small-map path while `medium`/`large` stay procedural. `base.png` is a local placeholder because `OPENAI_API_KEY` was unavailable for live image generation.
- Verified by `python3 /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/scripts/generate_small_map_assets.py`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`, and Playwright client screenshot pass against `http://127.0.0.1:4175`.
- Generated the missing manual mower directional sprite sheet at `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/push-mower-16dir.png` so the existing sprite lookup stops requesting a missing file.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all` and a Playwright client pass against `http://127.0.0.1:4175` with no `errors-0.json`.
- Extended the art-backed mowing-map asset pipeline to `medium` and `large`: added per-map art config, added shared `scripts/generate_map_assets.py` plus a `small` wrapper, generated guide/base/mow/collision assets and prompt specs for all maps, and switched docs/tests to the all-maps art-backed flow. Placeholder `base.png` files remain local/generated until `OPENAI_API_KEY` is available for live image generation.
- Verified by `python3 /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/scripts/generate_map_assets.py`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`, and Playwright client screenshot passes against `http://127.0.0.1:4175` for `medium` and `large` with no `errors-0.json`.
  - When `requireConfirm` is `false`, processing now auto-runs `onComplete` after timer expiry instead of waiting for an extra Enter press.
- End-of-day transition update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js`:
  - Set `requireConfirm: false` for the `nextDay()` transition (`Advancing to next day...`) so returning to day action no longer needs a second Enter.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Validation:
  - `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/processing.js` passed.
  - `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js` passed.
- Added RNG debug logging for lead generation/qualification rolls:
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js` now logs solicit chance rolls and per-lead follow-up qualification rolls to browser console.
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/jobs.js` now logs `randInt` rolls/results.
  - Added `DEBUG_ROLLS` toggle in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/constants.js` (currently `true`).
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Replaced narrow debug flag with log-level categories:
  - Removed `DEBUG_ROLLS` and added `LOG_LEVEL` in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/constants.js`.
  - Added `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/logging.js` with level-based logging (`DEBUG`, `INFO`, `WARN`, `ERROR`, `NONE`).
  - Updated roll logging in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/jobs.js` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js` to use `logDebug(...)`.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Expanded RNG debug logging coverage in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/jobs.js`:
  - `pick(...)` now logs roll, selected index, and selected value.
  - This completes debug logging for all random checks currently used in gameplay flow (`pick`, `randInt`, solicit chance, follow-up qualification chance).
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Debug visibility improvement in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js`:
  - `Follow Up Leads` now logs a start summary with raw vs qualified counts.
  - When no raw leads exist, logs per-lead skip reasons (`status=qualified`) so users can see why no roll occurred.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Added deterministic debug-log verification path:
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/logging.js` now mirrors emitted logs into `window.__tycoonLogs` (ring buffer).
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js` now exposes `debug_log_tail` in `render_game_to_text()` payload.
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon.sh` now:
    - uses longer action waits so processing callbacks complete,
    - asserts debug roll logs are present (`debug_roll_logs_found: true`),
    - fails verification if no roll logs are found.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Added browser-visible logging sanity checks in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`:
  - Startup `INFO` log on initialize.
  - Added `window.tycoonLogTest()` to emit INFO/DEBUG messages on demand and return recent buffered logs.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Startup import mismatch fix in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/game.js`:
  - Root cause: stale mixed ES module cache in Chrome could load mismatched versions (e.g., `logging.js` expecting `LOG_LEVEL` while cached `constants.js` lacked it).
  - Fix: added cache-busting query string to dynamic import of `src/index.js`.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Input handling update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/keyboard.js`:
  - While `processing` mode is active, `Enter` now immediately completes the current processing step even before `awaitingConfirm`, allowing fast skip through progress screens.
- Console copy update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - Processing phase now shows `Please wait... (Enter to skip)` before confirm-ready state.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Day-action cursor persistence update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/dayActions.js`:
  - Removed end-of-day reset of `state.actionCursor` during `nextDay()` so returning to `day_action` keeps the previously selected action highlighted.
  - New-game/reset flow still initializes cursor to first action via core state reset.
- Verification: Verified by node --check, verify-tycoon.sh, artifact output check.
- Regression framework upgrade for `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text`:
  - Added deterministic seed override support in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js` via URL query (`?seed=<int>`).
  - Added `seed` to `render_game_to_text()` payload.
  - Added test utility hooks in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`:
    - `window.setTycoonSeed(seed)` to reset with explicit seed.
    - `window.__tycoonTestSetLeads({count,status})` for deterministic lead fixture setup in regression scenarios.
- Added deterministic regression runner:
  - New script `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js`.
  - Implements 3 golden scenarios (`solicit_report`, `follow_up_report`, `mow_offer_accept`) with exact JSON baseline comparison.
  - Implements seed-matrix determinism check (runs each seed twice, requires identical summaries).
  - Writes summary artifact to `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/regression-tests/latest-summary.json`.
- Added deterministic seed-runner wrapper:
  - New script `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-seed-matrix.sh`.
- Added committed golden baselines:
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/golden/solicit_report.json`
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/golden/follow_up_report.json`
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/golden/mow_offer_accept.json`
- CI wiring:
  - Added `/Users/zkendall/projects/MowGrassMoMoney/.github/workflows/tycoon-regression.yml` to run syntax check + regression suite on push/PR for `tycoon-poc-text/**`.
- Tooling + docs:
  - Added `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/package.json` and lockfile with Playwright dev dependency and regression scripts.
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` with deterministic seed usage + regression commands.
- Verification: Verified by node --check, run-regression-tests.js, golden baseline check, pass validation.
- Output readability update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`:
  - Changed `render_game_to_text()` serialization to `JSON.stringify(payload, null, 2)` so Playwright-captured `state-*.json` files are pretty-printed.
- Verification: Verified by node --check.
- Report text simplification in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - `follow_up` report now shows only lead outcomes (`Leads qualified`, qualified/raw totals) and omits unrelated revenue/cost/net/job-breakdown lines.
  - `solicit` report now shows only lead outcomes/totals (`Leads generated`, lead totals) and omits unrelated financial/breakdown lines.
  - Detailed financial + job breakdown output remains for mowing and hardware-shop reports.
- Verification: Verified by node --check.
- Further solicit/follow-up report trim in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - Hidden `REGULAR CUSTOMER OFFERS` block for `solicit` and `follow_up` report activities.
  - These report screens now contain only lead-outcome fields relevant to the chosen action.
- Verification: Verified by node --check.
- Follow-up report wording update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - Replaced follow-up summary lines with requested concise format:
    - `You got # new leads!`
    - `Names: ...`
  - Removed follow-up total lines (`Qualified leads available`, `Raw leads remaining`) from report view.
- Verification: Verified by node --check.
- Solicit report wording simplification in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - Replaced solicit summary block with concise format:
    - `You got # new leads!`
    - `Names: ...`
  - Removed solicit total lines (`Total leads now`, `Qualified leads now`, `Raw leads now`) from report view.
- Verification: Verified by node --check.
- Report readability tweak in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/render/consoleView.js`:
  - Added blank-line spacing above and below `You got N new leads!` in both `follow_up` and `solicit` report views.
- Verification: Verified by node --check.
- Verification: Verified by run-regression-tests.js, pass result check, golden baseline check.
- Verification runner update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon.sh`:
  - Added optional `--headed` flag support to run the Playwright client with a visible browser (`--headless false`).
  - Added explicit verify log output showing browser mode (`headed` vs `headless`).
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Headed verify pacing update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon.sh`:
  - Added a dedicated headed execution path (Playwright inline runner) that uses real-time keypress delays so action steps are visible at human pace.
  - Kept existing fast headless flow unchanged via `web_game_playwright_client.js`.
  - Fixed headed Playwright module resolution by executing the inline runner from `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text`.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Verification: Verified by verify-tycoon.sh, Playwright run, artifact output check.
- Reverted default start-state behavior in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/state.js`:
  - Day-one now starts with empty leads again (original behavior).
- Added test-only start-state override in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`:
  - New URL override: `?start_state=all_actions` seeds one raw + one qualified lead at initialize time.
  - New hook: `window.__tycoonTestSetStartStateOverride(mode, applyNow)`.
    - `mode='all_actions'` enables immediate day-action path coverage.
    - `mode='default'`/`null` clears override and restores default start.
- Documentation sync:
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` deterministic hooks section with override usage.
- Verification: Verified by node --check.
- Refactor: moved start-state override to preset-based state construction in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/state.js`.
  - Added `START_STATE_DEFAULT` and `START_STATE_ALL_ACTIONS` modes.
  - `createInitialState(mode)` now selects a preset default-state object directly.
  - `resetCoreState(state, mode)` now applies the same preset path (no post-reset lead mutation).
  - Added `isValidStartStateMode(mode)` helper for mode validation.
- Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js` to use the preset mode path.
  - Removed ad-hoc `applyStartStateOverride()` mutation.
  - Start-state mode is now resolved first, then passed into `resetCoreState(state, mode)`.
  - Preserved query override (`?start_state=all_actions`) and test hook (`window.__tycoonTestSetStartStateOverride`).
- Verification: Verified by node --check, sanity check.
- Simplified start-state override architecture per feedback:
  - Replaced preset-clone/predicate helpers in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/state.js` with two explicit full-state builders:
    - `createDefaultInitialState()`
    - `createAllActionsInitialState()`
  - `createInitialState(mode)` now selects one of those full objects directly.
  - `resetCoreState(state, mode)` now rehydrates from `createInitialState(mode)` and preserves runtime seed/rng while incrementing processing token.
- Simplified mode branching in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`:
  - Removed generic start-state validation helper usage.
  - Kept a single explicit check for `all_actions`; all other values fall back to `default`.
- Verification: Verified by node --check, sanity check.
- Start-state override mode rename and expansion:
  - Renamed explicit test mode from `all_actions` to `test_all_actions`.
  - Replaced lead-only override with a full mid-game snapshot in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/state.js`:
    - day/cash/tier advanced (`day=6`, upgraded mower tier).
    - populated repeat customers, mixed raw/qualified leads, planning jobs, selected jobs, accepted jobs.
    - populated prior report, pending regular-customer offers, selected offer cursor/state.
    - non-default cursors/input values and note/ticks to reflect in-progress progression.
- Wiring updates in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`:
  - `?start_state=test_all_actions` now activates the explicit mid-game snapshot.
  - `window.__tycoonTestSetStartStateOverride('test_all_actions', true)` now uses the renamed mode.
- Documentation sync:
  - Updated deterministic hook docs in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` to use `test_all_actions` and describe the full mid-game snapshot.
- Verification: Verified by node --check, sanity check.
- Verification: Verified by verify-tycoon.sh, artifact output check.
- Verification script consolidation:
  - Removed `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon.sh` wrapper.
  - Kept single implementation script: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh`.
- Verification: Verified by verify-tycoon-quick.sh.
- Expanded verification choreography in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh` to cover additional day actions.
  - Verify sequence now explicitly attempts: `Solicit` -> `Follow Up Leads` -> `Mow Lawns` -> `Shop for New Hardware` in one run.
  - Updated both headless action JSON and headed keypress path.
  - Reduced headless iterations from 2 to 1 because the single choreography now spans multiple day transitions/actions.
- Verification: Verified by verify-tycoon-quick.sh.
- Verification script readability refactor:
  - Extracted headless choreography payload from inline `--actions-json` into `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-actions.json`.
  - Extracted headed Playwright inline `node -e` logic into `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js`.
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh` to reference the extracted files via `--actions-file` and runner script invocation.
- Verification: Verified by verify-tycoon-quick.sh.
- Progress log cleanup in `/Users/zkendall/projects/MowGrassMoMoney/progress.md`:
  - Replaced verbose verification sub-bullets with concise single-line summaries in the form `Verification: Verified by ...`, inferred from each original verification block.
- Progress log format correction in `/Users/zkendall/projects/MowGrassMoMoney/progress.md`:
  - Normalized remaining `Verification: Ran ...` lines to `Verification: Verified by ...` for consistency with the new concise style.
- Verification:
  - Ran `./tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174 --headed`.
  - Artifacts: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/40-no-change-web-game` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/40-no-change-probe.json`.
- Documentation sync in `/Users/zkendall/projects/MowGrassMoMoney/AGENTS.md`:
  - Added explicit brevity rules for verification logging in `progress.md` (single-line `Verified by ...`, no artifact paths unless needed for failures/investigations, avoid no-change rerun noise).
- Verify choreography reliability fix after observed missing `mow`/`shop_hardware` coverage:
  - Root cause: entering planning from `mow` clears `selectedJobIds`, so `Enter` alone did not advance to performance; sequence stalled in planning.
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-actions.json` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js` to select a job (`Space`) before planning confirm.
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh` to default verify URL to `?start_state=test_all_actions` when not provided and to include `last_report_activity` in probe output.
  - Relaxed probe guard so missing `roll=` debug logs no longer hard-fail verify runs in mid-game test starts.
- Verification:
  - Ran `./tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174` after updates.
  - Artifacts: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/43-verify-tycoon-actions-web-game` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/43-verify-tycoon-actions-probe.json`.
- Refactored `tycoon-poc-text/scripts/verify-tycoon-quick.sh` to replace long inline `node -e` blocks with `tycoon-poc-text/scripts/compute-verify-label.js` and `tycoon-poc-text/scripts/summarize-verify-states.js`.
- Verified by `bash -n tycoon-poc-text/scripts/verify-tycoon-quick.sh`, `node --check tycoon-poc-text/scripts/compute-verify-label.js`, and `node --check tycoon-poc-text/scripts/summarize-verify-states.js`.
- Verification:
  - Ran `./tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174 --headed` with `test_all_actions` start override.
  - Probe confirms sequence reached hardware shop path (`last_report_activity: "shop_hardware"`).
  - Artifacts: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/44-verify-web-game` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/44-verify-probe.json`.
- Verification input tuning for mow profitability:
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-actions.json` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js` so mow planning now selects multiple jobs before confirming (three selections via `Space`).
- Verification:
  - Ran `./tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174`.
  - Artifacts: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/45-verify-tycoon-actions-web-game` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/45-verify-tycoon-actions-probe.json`.
  - Captured state shows increased post-mow cash (`cash: 823`), which is above riding mower upgrade cost (`790`).
- Verification:
  - Ran `./tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174 --headed` with multi-job mow sequence.
  - Probe confirms flow ended in hardware shop report (`last_report_activity: "shop_hardware"`).
  - Artifacts: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/46-no-change-web-game` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/46-no-change-probe.json`.
- Verify flow correction for explicit full-action walkthrough starting from `solicit` cursor:
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/state.js` test preset so `test_all_actions` starts with `actionCursor: 0` (`Solicit`).
  - Replaced brittle timing-only verify choreography with deterministic state-aware runner logic in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js`.
    - Runner now explicitly drives: `Solicit` -> `Follow Up Leads` -> `Mow Lawns` (selects multiple jobs) -> `Shop for New Hardware` (buy path).
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh` to use the same walkthrough runner for both headless and headed modes.
- Verification:
  - Ran `./tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174`.
  - Artifacts: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/49-verify-web-game` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/49-verify-probe.json`.
  - Probe confirms walk-through reached hardware report (`last_report_activity: "shop_hardware"`) while ending in report mode.
- Documentation enhancement in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-actions.json`:
  - Added `desc` field to every step to explain the intended action/phase transition while preserving the existing input sequence.
- Verification:
  - Parsed `verify-tycoon-actions.json` with Node JSON parser (`ok`).
- Added maintainability notes to verification test files for readability expectations:
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh`
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js`
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-actions.json`
  - Guidance states test steps should remain clean, explicit, and documented (`desc`) for legibility.
- Verification:
  - Parsed `verify-tycoon-actions.json` (`json ok`).
  - Ran `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js`.
- Regression suite readability refactor in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js`:
  - Added generic step-plan executor (`executePlan`) with per-step failure context including `desc`.
  - Added reusable step ops for explicit, state-aware interactions: mode assertions/waits, key presses, lead seeding, processing completion, and key scenario assertions.
  - Scenario implementations now run named plans instead of inline imperative keypress blocks.
- Added step-plan definitions file:
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/regression-step-plans.json`.
  - Contains documented (`desc`) plans for `solicit_report`, `follow_up_report`, `mow_offer_accept`, and `seed_summary`.
- Verification:
  - Ran `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js`.
  - Parsed `regression-step-plans.json` (`step plans json ok`).
- Regression runner update in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js`:
  - Added headed toggle support.
  - New CLI options:
    - `--headed` sets Playwright launch to headed (`headless: false`).
    - `--headless true|false` (or `1|0`) explicitly controls headless mode.
  - Runner summary output now includes `headless` to show launch mode used.
- Verification:
  - Ran `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js`.
- Verification:
  - Ran headed regression suite: `npm --prefix tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174 --headed`.
  - Result: pass (`status: ok`) across scenarios `solicit_report`, `follow_up_report`, and `mow_offer_accept`.
  - Summary artifact: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/output/regression-tests/latest-summary.json`.
- Regression test scope simplification:
  - Removed seed-matrix-only pathway from tooling/docs and deleted `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-seed-matrix.sh`.
  - Added deterministic RNG unit test script: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/test-rng-determinism.js`.
  - Updated scripts/docs/CI to use `test:rng` + scenario regressions (`package.json`, `README.md`, `.github/workflows/tycoon-regression.yml`).
  - Removed unused `seed_summary` plan from `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/regression-step-plans.json`.
- Verification: Verified by `node --check` (updated scripts), `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:rng`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`.
- Documentation: Added tycoon system design diagrams in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md` and linked from `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md`.
- Documentation: Updated verification Mermaid labels in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md` to avoid lexical parse failures.
- Verify runner migration toward JS-first execution:
  - Added `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.js` as the primary quick-verify orchestrator (arg parsing, default test start-state URL handling, label/history integration, syntax check, Playwright run, probe summary, history persistence).
  - Switched output run IDs from incremental counters to UTC timestamps (`<timestamp>-<label>-*`).
  - Refactored `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/compute-verify-label.js` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/summarize-verify-states.js` to expose reusable functions for the JS orchestrator while preserving CLI behavior.
  - Reduced `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh` to a compatibility wrapper that delegates to the JS runner.
  - Added npm script `verify:quick` and updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` verify docs for JS entrypoint + timestamped artifacts.
- Verification: Verified by `node --check` (verify scripts), `bash -n /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh`, and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh http://127.0.0.1:4174`.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:syntax`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:rng`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174`.
- Verification: Measured test suite runtimes (`/usr/bin/time -p`) for `verify:syntax`, `test:rng`, `test:regression`, and `verify:quick` against `http://127.0.0.1:4174`.
- Documentation: Updated /Users/zkendall/projects/MowGrassMoMoney/AGENTS.md progress-log rules to skip progress.md updates for verification-only runs and clarified verification-note format text.
- Quick verify headless pacing optimization in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js`:
  - Removed human-action delays in headless mode while preserving headed pacing.
  - Reduced mode polling delay in headless mode.
  - Added explicit wait for `render_game_to_text` readiness after navigation to avoid startup race without reintroducing fixed waits.
- Verification: Verified by `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js` and timed run `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174` (`real 8.72s`).
- Consolidated test orchestration into `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js` while keeping suites separate.
  - Added `--suite regression|quick|all` support.
  - Moved quick-verify orchestration (label/history, timestamped artifacts, syntax check, walkthrough execution, probe summary) into the shared runner.
  - Kept compatibility wrappers by delegating `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.js` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh` to the consolidated runner.
  - Updated `verify:quick` npm script and README usage notes to reflect suite-based runner usage.
- Verification: Verified by `node --check` (runner/wrapper scripts), `bash -n /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh`, `npm --prefix tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`, and `npm --prefix tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174`.
- Verification labeling update: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/compute-verify-label.js` now classifies `scripts/run-regression-tests.js` changes under `verify` category.
- Verification: Verified by `node --check` (label/runner scripts), `npm --prefix tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`, and `npm --prefix tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174`.
- Quick suite correction: restored quick verification flow and removed only compute-label functionality.
  - `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js` no longer imports/uses `compute-verify-label.js` or `.verify-history.json` hashing.
  - Quick artifacts now use fixed `verify` label: `<timestamp>-verify-web-game` and `<timestamp>-verify-probe.json`.
  - Removed obsolete script: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/compute-verify-label.js`.
  - Updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md` to remove hash-based labeling/history references.
- Verification: Verified by `node --check` (runner/wrapper scripts), `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:syntax`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:rng`, `npm --prefix tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`, and `npm --prefix tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174`.
- Documentation planning artifact added in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/PLAYWRIGHT-TEST-MIGRATION-PLAN.md` with current regression/quick/RNG scenario baselines and a post-migration Playwright Test coverage parity checklist.
- Verification: Verified by `rg -n "^# Playwright Test Migration Plan|^## Detailed Existing Scenarios|^## Post-Migration Coverage Parity Checklist" /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/PLAYWRIGHT-TEST-MIGRATION-PLAN.md`.
- Migrated regression coverage to Playwright Test using `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/playwright.config.js`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/regression.spec.js`, and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-playwright-regression.js`; kept quick verify on `run-regression-tests.js --suite quick`.
- Updated test docs and pipeline wiring in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md`, `/Users/zkendall/projects/MowGrassMoMoney/.github/workflows/tycoon-regression.yml`, and recorded parity status in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/PLAYWRIGHT-TEST-MIGRATION-PLAN.md`.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:regression:update -- --url http://127.0.0.1:4174`.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:rng`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:syntax`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174`.
- Unified test framework by migrating quick verify to Playwright Test: added `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/quick-verify.spec.js` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-playwright-quick-verify.js`, switched `verify:quick`, and simplified `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js` into a compatibility delegator.
- Documentation sync for unified quick/regression runner paths in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md`, and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/PLAYWRIGHT-TEST-MIGRATION-PLAN.md`.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:regression -- --url http://127.0.0.1:4174`.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174`.
- Verification: Verified by `node /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js --suite all --url http://127.0.0.1:4174`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:syntax`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:rng`.
- Fixed headed quick-verify console-error false failure by adding an explicit no-op favicon link (`<link rel="icon" href="data:,">`) in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/index.html` to prevent `/favicon.ico` 404 requests.
- Verification: Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:quick -- --url http://127.0.0.1:4174 --headed`.
- Simplified Playwright command flow to direct CLI usage (`npx playwright test`) for regression and quick-verify; updated npm scripts, CI step, compatibility delegators, and test docs; removed `scripts/run-playwright-regression.js` and `scripts/run-playwright-quick-verify.js`.
- Verified by `npm --prefix tycoon-poc-text run verify:syntax`, `npm --prefix tycoon-poc-text run test:rng`, `cd tycoon-poc-text && TYCOON_BASE_URL=http://127.0.0.1:4174 npx playwright test tests/regression.spec.js --config=playwright.config.js`, `cd tycoon-poc-text && node scripts/run-regression-tests.js --suite quick --url http://127.0.0.1:4174`, and `cd tycoon-poc-text && TYCOON_BASE_URL=http://127.0.0.1:4174 npm run verify:quick`.
- Removed obsolete legacy test wrappers/scripts after Playwright cutover: `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/run-regression-tests.js`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.js`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-quick.sh`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-headed-runner.js`, and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/scripts/verify-tycoon-actions.json`.
- Synced cleanup docs in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/PLAYWRIGHT-TEST-MIGRATION-PLAN.md` for direct Playwright-only entrypoints.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run verify:syntax`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text run test:rng`, `cd /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text && TYCOON_BASE_URL=http://127.0.0.1:4174 TYCOON_DISABLE_WEBSERVER=1 npx playwright test tests/regression.spec.js --config=playwright.config.js`, and `cd /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text && TYCOON_BASE_URL=http://127.0.0.1:4174 TYCOON_DISABLE_WEBSERVER=1 npx playwright test tests/quick-verify.spec.js --config=playwright.config.js`.
- Renamed game branding to `MoCrassMoMoney` across root README, both POC UIs (HTML titles/headings + mowing start overlay), and POC/art document headings.
- Corrected game branding from `MoCrassMoMoney` to `MoGrassMoMoney` across root README, both POC UIs (HTML titles/headings + mowing start overlay), and POC/art document headings.
- Documentation: added quick-verify trace recording/replay commands (`--trace on`, `show-trace`) to `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md`.
- Simplified local/CI test invocation docs by removing redundant `TYCOON_BASE_URL=http://127.0.0.1:4174` overrides from `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` and removed the same redundant env override from `/.github/workflows/tycoon-regression.yml`; kept explicit note that `TYCOON_BASE_URL` is only for non-default targets.
- Verified by `rg -n "TYCOON_BASE_URL=http://127.0.0.1:4174" /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md /Users/zkendall/projects/MowGrassMoMoney/.github/workflows/tycoon-regression.yml`.
- Test-layout clarity refactor in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text`: moved test-only helpers from `scripts/` into `tests/` (`tests/fixtures/regression-step-plans.json`, `tests/helpers/summarize-verify-states.js`, `tests/test-rng-determinism.js`), rewired imports in regression/quick specs, and updated `test:rng` script path in `package.json`.
- Verified by `node --check tycoon-poc-text/tests/regression.spec.js`, `node --check tycoon-poc-text/tests/quick-verify.spec.js`, `node --check tycoon-poc-text/tests/helpers/summarize-verify-states.js`, and `npm --prefix tycoon-poc-text run test:rng`.
- Documentation: consolidated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` test docs by merging `Snapshot Naming Guideline` and `Regression Tests` into a single `Testing` section.
- Documentation: reorganized `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` `Testing` section with subheaders and concise prose context (`Setup`, `Common Commands`, `Trace Recording and Replay`, `Artifacts and Notes`) to replace the flat list layout.
- Increased headed Playwright pacing for readability in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/quick-verify.spec.js` (default action delays and explicit per-step waits) and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/regression.spec.js` (headed-only delay between scripted keypresses).
- Verified by `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/quick-verify.spec.js` and `node --check /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/regression.spec.js`.
- Test-driver refactor: quick verify now uses fixture-driven scenario config in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/fixtures/quick-verify-step-plans.json` (seed/start_state/steps) with JSON step execution in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/quick-verify.spec.js`.
- Regression fixture model now includes per-scenario `seed` + `start_state` in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/fixtures/regression-step-plans.json`; added pre-coded start states `test_regression_follow_up` and `test_regression_mow_offer_accept` in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/state.js` and wired validation/override parsing in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/src/index.js`.
- Documentation sync: updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md` for fixture-driven suites and available `start_state` modes.
- Verified by `node --check tycoon-poc-text/tests/regression.spec.js`, `node --check tycoon-poc-text/tests/quick-verify.spec.js`, `node --check tycoon-poc-text/src/state.js`, `node --check tycoon-poc-text/src/index.js`, `npm --prefix tycoon-poc-text run test:regression`, and `npm --prefix tycoon-poc-text run verify:quick`.
- Documentation: added `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/GAME-DESIGN.md` as the canonical incremental game-design spec (design lens, work trees, streams, numeric rules, and feature-expansion protocol), and linked it from `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md`.
- Verified by `rg -n "## Game Design|GAME-DESIGN.md|## Current Numeric Rules|## Work Trees" /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/GAME-DESIGN.md`.
- Documentation: refactored `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/GAME-DESIGN.md` around an explicit upgrade lens, added upgrade-tree framing (hardware current; solicitation/qualification/crew planned), and updated next feature ideas to prioritize those upgrade tracks.
- Verified by `rg -n "## Upgrade Lens|### Upgrade Trees|Solicitation Upgrades|Lead Qualification Upgrades|Crew Building and Management|each major day action" /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/GAME-DESIGN.md`.
- Documentation: added top-level `Mini-Game Lens` section to `/Users/zkendall/projects/MowGrassMoMoney/Summary.md`, focused on `Solicit` (flyer drop, drive-by flyer throw, house targeting) and `Mow`, with other action lanes explicitly parked for now.
- Verified by `rg -n "^## Mini-Game Lens|Flyer Drop|Drive-By Flyer Throw|### Parked for Now" /Users/zkendall/projects/MowGrassMoMoney/Summary.md`.
- Reworked `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` from real-time steering to draw/review/animate path gameplay with brush+centerline overlays, clickable `Accept`/`Retry` review controls, playback path smoothing + lawn-boundary clamping, obstacle crash flips with `-$1` popups, crash economy tracking, and expanded deterministic `render_game_to_text`/`advanceTime` test contract fields.
- Added Playwright harness for mowing path mechanics in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/playwright.config.js`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/helpers/game-driver.js`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/crash-economy.spec.js`, and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-overlay.spec.js`; updated scripts in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/package.json` plus control text in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Documentation: added a planned neighborhood-navigation design lens in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/GAME-DESIGN.md` (house-marked map selection + owner-crew control with abstract dispatch scaling) and synced framing into `/Users/zkendall/projects/MowGrassMoMoney/Summary.md` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/POC-Tycoon.md` open questions.
- Verified by `rg -n "Neighborhood Navigation Lens|neighborhood_map|Neighborhood Day Map and Routing|Neighborhood Navigation|neighborhood day-map|abstract dispatch" /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/GAME-DESIGN.md /Users/zkendall/projects/MowGrassMoMoney/Summary.md /Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/POC-Tycoon.md`.
- Updated mowing playback behavior in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: animation now runs the full accepted path before win evaluation, playback view now hides brush swath and draws a slightly smoothed centerline, and hold-`Space` fast-forward was added (with exposed `fast_forward` and effective playback speed in `render_game_to_text`); synced control copy in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md`, and added fast-forward coverage test in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Adjusted crash sensitivity and playback line styling in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: crash overlap checks now use the centerline position (`radius=0`) instead of mower body radius, and animation path line now renders as a black dashed smoothed centerline.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Documentation sync: updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` to reflect current draw/review/animate behavior, end-of-route win check, fast-forward control, black dashed playback line, persistent progress across runs, and line-overlap crash penalties.
- Verified by `rg -n "drive mower|steer|right mouse|reverse|Space|Accept|Retry|black dashed|centerline|95%|Crash|line-overlap|persist" /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md`.
- Added explicit no-op favicon link (`<link rel="icon" href="data:,">`) to `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html` to prevent `/favicon.ico` 404 requests.
- Verified by `rg -n "rel=\"icon\"" /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html`.
- Added mower fuel tank behavior in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: fuel capacity/state now tracks on mower, drains during playback distance movement, displays in HUD, and is exposed in `render_game_to_text`; added fuel-drain assertion in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js` and synced docs in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Added long full-playthrough test coverage in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/long-playthrough.spec.js` with fixture-driven scenario `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/fixtures/long-playthrough-step-plans.json`; flow starts from default state and loops `Solicit` until raw leads exist, loops `Follow Up Leads` until qualified leads exist, then completes mow conversion (offer acceptance to repeat customer) and hardware-shop report.
- Added npm entrypoint `verify:long` in `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/package.json` and updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` plus `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md` for long-suite docs.
- Verified by `node --check tycoon-poc-text/tests/long-playthrough.spec.js`, `node -e "JSON.parse(require('fs').readFileSync('tycoon-poc-text/tests/fixtures/long-playthrough-step-plans.json','utf8')); console.log('long fixture json ok');"`, `npm --prefix tycoon-poc-text run verify:long`, `npm --prefix tycoon-poc-text run test:regression`, and `npm --prefix tycoon-poc-text run verify:quick`.
- Made mower fuel type-contingent in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` (`push` uses no fuel, `small` uses a 0.5 gal tank), added paid refills at `$3.00/gal` on `E`, blocked/paused playback when fuel is empty until refilled, exposed mower/fuel economy fields in `render_game_to_text`, and synced behavior/tests/docs in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md`, and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Tuned `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` fuel behavior so small-mower burn is slower (`fuelBurnPerPixel` `0.0002`), fuel-empty now pauses in `animating` without clearing playback, `E` refills during animation and resumes the same path from current progress, and `render_game_to_text` now exposes `playback.waiting_for_fuel`; synced resume wording in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md`, and updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js` to assert pause/refill/resume flow.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Refactored tycoon Playwright tests to a shared harness under `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/harness` (`scenario.js`, `runtime.js`, `steps.js`, `artifacts.js`) and migrated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/regression.spec.js`, `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/quick-verify.spec.js`, and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/tests/long-playthrough.spec.js` to use it while keeping suite-specific ops/assertions local.
- Documentation sync: updated `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/tycoon-poc-text/SYSTEM-DESIGN.md` to document shared harness boundaries.
- Verified by `node --check tycoon-poc-text/tests/harness/scenario.js`, `node --check tycoon-poc-text/tests/harness/runtime.js`, `node --check tycoon-poc-text/tests/harness/steps.js`, `node --check tycoon-poc-text/tests/harness/artifacts.js`, `node --check tycoon-poc-text/tests/regression.spec.js`, `node --check tycoon-poc-text/tests/quick-verify.spec.js`, `node --check tycoon-poc-text/tests/long-playthrough.spec.js`, `npm --prefix tycoon-poc-text run test:regression`, `npm --prefix tycoon-poc-text run verify:quick`, and `npm --prefix tycoon-poc-text run verify:long`.
- Added required setup menu + configurable gameplay presets in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: new `menu` mode with mouse/keyboard selection flow, mower registry (`manual`, `small_gas`, `large_rider`) with tiered speed/deck/fuel behavior, three distinct lawn-map registries (`small`, `medium`, `large`) with unique scene geometry/obstacles, session-only setup retention across `R`, and expanded `render_game_to_text` setup payload (`setup` metadata, menu button/option hitboxes, selected map id).
- Updated mowing Playwright harness/specs to be menu-driven: `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/helpers/game-driver.js` (`selectMower`, `selectLawn`, `clickStartJob`, `setupFromMenu`), revised `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js`, revised `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/crash-economy.spec.js` with per-lawn crash coverage, updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-overlay.spec.js`, and added `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/menu-setup.spec.js`.
- Synced menu/mower/map behavior docs and UI copy in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md`, and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Extracted mowing map generation/registry from `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` into new `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/maps.js` (`createMowingLawnMaps` + `window.MOWING_LAWN_MAPS`), updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` to consume external map definitions with load guard, and wired `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/index.html` to load `maps.js` before `game.js`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Updated mower visual mapping in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` to use per-type sprite frames from `assets/mower-sheet.png`: `manual` now uses the push mower frame, `small_gas` keeps the red gas frame, and `large_rider` now uses the orange seated mower frame (row 2, column 3; `x=512`, `y=256`); frame selection is applied when setup choices are applied.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Refined mower sprite mapping in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: large rider now uses a tight crop of the orange seated mower frame to avoid neighboring-sprite bleed (`x=537`, `y=348`, `w=192`, `h=164`), and mower types now set per-type draw size (`spriteDraw`) so the large rider renders larger to better match its wider brush deck.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Converted `small` lawn to circular geometry with deterministic spawn in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/maps.js`, added non-mowable yard features (pool + cement paths), and updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` to use shape-aware lawn checks/clamping/rendering plus `render_game_to_text` map metadata (`lawn.kind` + `yard_features`) while keeping medium/large behavior unchanged.
- Updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/crash-economy.spec.js` for shape-aware path generation and added a small-map non-mow-zone no-crash assertion; adjusted `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-overlay.spec.js` outside-pixel tolerance to reduce false failures.
- Synced docs in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md` for circular small-map + non-mow-yard behavior.
- Verified by `node --check /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`, `node --check /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/maps.js`, `node --check /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/path-flow.spec.js`, `node --check /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/crash-economy.spec.js`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- HUD clarity update in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: replaced overlapping center overlays with a bottom instruction toast (`drawInstructionToast`), added compact top-left status bar (`Coverage/Target/Cash/Fuel`), removed fixed right-side status fields (`Music/Crashes/Mode/Refill`), and moved review-mode toast above buttons to avoid overlap.
- Verified by `node --check /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Replaced `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/{small,medium,large}/base.png` placeholder map art with live `imagegen` outputs via repo-local venv `/Users/zkendall/projects/MowGrassMoMoney/tmp/imagegen-venv`; mow/collision masks and gameplay sources remain unchanged.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all` and Playwright client browser passes against `http://127.0.0.1:4175` for `small`, `medium`, and `large`.
- Added stripe-aware mowing in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: mow cells now store a light/dark stripe variant chosen from mower heading plus perpendicular cell offset, and rendering uses alternating `grass-mowed-light.png` / `grass-mowed-dark.png` tiles instead of one flat mowed texture.
- Added deterministic grass asset generator `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/scripts/generate_grass_assets.py`, regenerated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-unmowed.png`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-mowed-light.png`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-mowed-dark.png`, and compatibility tile `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-mowed.png`, and added visual regression coverage in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-stripes.spec.js`.
- Synced `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/package.json` for the new grass asset pipeline and visual test scope.
- Verified by `python3 mowing-poc-game/scripts/generate_grass_assets.py`, `python3 -m py_compile mowing-poc-game/scripts/generate_grass_assets.py`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual`, and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`.
- Corrected stripe selection in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`: mowed tone is now chosen from pass direction only, so a single lane stays consistent and the return lane supplies the contrast; updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-stripes.spec.js` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` to match.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual` and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`.
- Flipped direction-to-tone mapping in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` so upward travel stamps the light mow tile and downward travel stamps the dark mow tile; updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-stripes.spec.js` to use lane-average sampling with thresholds that match the current grass art contrast.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual` and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`.
- Replaced binary mow-tone stamping in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` with continuous per-cell lay blending: cells now accumulate heading-driven light/dark bias across repeated passes and render by crossfading the light/dark mowed grass tiles; tuned `layBlendStrength` down so repeated passes change tone gradually instead of saturating in one sweep.
- Updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/visual-stripes.spec.js` to assert repeated same-direction and opposite-direction passes keep shifting lane tone, and synced `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` for the new gradual shading behavior.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual` and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`.
- Tightened grass brightness contrast in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js`, switched grass rendering to deterministic rotated sub-tiles per mow cell, and kept density at 2x2 sub-tiles (quarter-area tiles) to reduce repetition without breaking playback performance; near-full refill need now snaps to zero within a small epsilon.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all` and a Playwright browser snapshot pass against `http://127.0.0.1:4175`.
- Changed `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` grass rendering to one sprite per mow cell with `mowGrid.cell` reduced from `18` to `9` (half-width/height), while keeping the tighter mowed brightness range; each unmowed and mowed cell now draws an opaque color fill before sprite overlays so authored map art cannot show through grass alpha.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all` and a Playwright browser pass against `http://127.0.0.1:4175`.
- Replaced `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-unmowed.png`, `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-mowed-light.png`, and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-mowed-dark.png` with live OpenAI image outputs resized into the game asset slots; these now use random-blade textures instead of locally generated patterned tiles.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all` and a Playwright browser pass against `http://127.0.0.1:4176`.
- Generated style-pivot mockups from the current mowing map art into `/Users/zkendall/projects/MowGrassMoMoney/output/imagegen/style-pivots` and documented the directions in `/Users/zkendall/projects/MowGrassMoMoney/art/StylePivotMockups.md`; strongest candidates are `small-flat-indie.png` / `medium-flat-indie.png` and `small-bold-cartoon.png`, with `small-retro-lite.png` kept as a weaker boundary check.
- Documentation sync for the chosen `Flat Indie` art direction: updated `/Users/zkendall/projects/MowGrassMoMoney/art/ArtDirection.md`, `/Users/zkendall/projects/MowGrassMoMoney/art/PromptPack.md`, `/Users/zkendall/projects/MowGrassMoMoney/art/MoodBoards.md`, `/Users/zkendall/projects/MowGrassMoMoney/art/ConceptShotlist.md`, `/Users/zkendall/projects/MowGrassMoMoney/Summary.md`, and the three `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/*/base-imagegen-prompt.md` files to make flat-indie the default and remove old painterly/isometric guidance.
- Regenerated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/{small,medium,large}/base.png` from the live scene layouts in the chosen flat-indie direction using image edits, with staging outputs kept in `/Users/zkendall/projects/MowGrassMoMoney/output/imagegen/map-regen`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Corrected the flat-indie map pass after review: `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/small/base.png` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/medium/base.png` now use the previously approved flat-indie mockups directly, and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/large/base.png` was regenerated with crisper outlines and flatter shading to match; staging render kept at `/Users/zkendall/projects/MowGrassMoMoney/output/imagegen/map-regen/large-flat-indie-crisp.png`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Refined `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/large/base.png` again to remove the remaining dappled lawn texture and better match the flatter medium-map treatment; final staging render kept at `/Users/zkendall/projects/MowGrassMoMoney/output/imagegen/map-regen/large-flat-indie-match-medium.png`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Added `empty_field` as a fourth mowing map in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/maps.js` with no obstacles or yard features for grass/mechanics experimentation, generated plain runtime assets under `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/maps/empty-field`, updated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/README.md` and `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/POC-Mowing.md`, and expanded `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/menu-setup.spec.js` to cover the sandbox map plus dynamic map-art checks.
- Verified by `node --check /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/maps.js` and `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:all`.
- Refactor: split `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/game.js` into a thin loader plus `mowing-poc-game/src` domain/render/input modules, preserved `window.render_game_to_text()` / `window.advanceTime(ms)`, added a README `Code Structure` section, and added a boot-payload regression assertion in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/menu-setup.spec.js`. Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:quick`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual`, `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game test`, and the Playwright web-game client against `http://127.0.0.1:4175`.
- Reworked `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game` grass rendering to a 16px autotile grid: replaced the old three-texture lay-blend path with a single `2x8` grass sprite sheet, added neighbor-mask frame selection plus reverse-order skirt rendering in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/render/scene.js`, removed obsolete lay-blend state, added `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-autotile-sheet-prompt.md`, and generated `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-autotile-sheet.png` from imagegen (raw source archived to `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/output/imagegen/grass-autotile-sheet-raw.png` before packing to the runtime atlas).
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual` and `node /Users/zkendall/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js --url http://127.0.0.1:4173 --iterations 1 --screenshot-dir /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/output/web-game-grass-autotile --actions-json '{"steps":[{"buttons":["left_mouse_button"],"frames":18,"mouse_x":420,"mouse_y":200},{"buttons":[],"frames":4},{"buttons":["left_mouse_button"],"frames":22,"mouse_x":420,"mouse_y":520},{"buttons":[],"frames":8}]}'`.
- Cleaned the grass autotile follow-up in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game`: made the `4px` overlap explicit in debug/config, tightened `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-autotile-sheet-prompt.md` to forbid dirt/boxed borders, and post-processed `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/assets/grass-autotile-sheet.png` to remove all brown soil pixels and flatten no-south/no-east interior variants so the lawn tiles without horizontal banding.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual` and `node /Users/zkendall/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js --url http://127.0.0.1:4173 --iterations 1 --screenshot-dir /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/output/web-game-grass-autotile-clean2 --actions-json '{"steps":[{"buttons":[],"frames":6}]}'`.
- Narrowed the manual push mower cut width in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/src/constants.js` by reducing `deckRadius` from `22` to `16` so the mowed strip and route brush better match the visible deck on `push-mower-16dir.png`; added a guard assertion in `/Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/tests/menu-setup.spec.js`.
- Verified by `npm --prefix /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game run test:visual` and `node /Users/zkendall/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js --url http://127.0.0.1:4173 --iterations 1 --screenshot-dir /Users/zkendall/projects/MowGrassMoMoney/mowing-poc-game/output/web-game-push-width --actions-json '{"steps":[{"buttons":[],"frames":4,"mouse_x":240,"mouse_y":245},{"buttons":["left_mouse_button"],"frames":2,"mouse_x":240,"mouse_y":245},{"buttons":[],"frames":4,"mouse_x":240,"mouse_y":245},{"buttons":[],"frames":4,"mouse_x":250,"mouse_y":376},{"buttons":["left_mouse_button"],"frames":2,"mouse_x":250,"mouse_y":376},{"buttons":[],"frames":4,"mouse_x":250,"mouse_y":376},{"buttons":[],"frames":4,"mouse_x":380,"mouse_y":525},{"buttons":["left_mouse_button"],"frames":2,"mouse_x":380,"mouse_y":525},{"buttons":[],"frames":8,"mouse_x":380,"mouse_y":525},{"buttons":[],"frames":4,"mouse_x":260,"mouse_y":160},{"buttons":["left_mouse_button"],"frames":2,"mouse_x":260,"mouse_y":160},{"buttons":["left_mouse_button"],"frames":20,"mouse_x":260,"mouse_y":500},{"buttons":[],"frames":8,"mouse_x":260,"mouse_y":500}]}'`; `MOWING_DISABLE_WEBSERVER=1 npx playwright test tests/menu-setup.spec.js --config=playwright.config.js` was blocked by sandboxed Chromium launch permissions.
